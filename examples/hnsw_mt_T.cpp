...
int main() {
    ...
    // std::vector<std::vector<float>> Paulis_3(64);
    // for(int i = 0; i < 64; i++) {
    //     Paulis_3[i] = tensor(Paulis_2[i>>2], Paulis[i&3]);
    // }
    ...
    std::string prefix = std::string("data/hnsw_mt_T")+std::to_string(D)+"_";

    // multi threads
    QSY::hnsw<float, D> hnsw(t2._data, t2._bound, {
                            .hnsw_graph_prefix = prefix,
                            .num_thread_construct = 20,
                            .num_thread_search = 20
                        });
